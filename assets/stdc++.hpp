...
#define function_alias(name,call) constexpr struct { template<class... Args> auto operator()(Args&&... args) const -> decltype(call(std::forward<Args>(args)...)) { return call(std::forward<Args>(args)...); } } inline name [[maybe_unused]]
