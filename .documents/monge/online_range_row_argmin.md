...
### Order
構築$\order{W log H}$、クエリ$\order{\log^2 W}。$
ただし、クエリでのselect評価回数は$\order{log W}に抑えられている。$
...
### Constructor
- `online_range_row_argmin(int H,int W,Guide)`: Guideにはselectが定義されていることが要件。
- `online_range_row_argmin(int H,int W,Guide,Comp)`: Guideにはvalueが定義されていることが要件。Compを用いて比較する
- `online_range_row_argmin(int H,int W,Guide)`: Guideにはvalueが定義されていることが要件。lessを用いて比較する。

### Member
#### Function
- `build() -> void`: 構築する。
- `row_argmin(T x,int l,int r) -> int`: $x$行目の区間行最小値を求める。
- `row_min_value(T x,int l,int r) -> U`: $x$行目の区間行最小値を求める。
